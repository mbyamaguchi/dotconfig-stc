...
-- ts_ls の typescript / javascript 共通インレイヒント設定
...
return {
  ...
  -- 0.12 のネイティブ LSP では lspconfig 自体は必須ではないが、各サーバーの
  -- lsp/<name>.lua（filetypes・root_markers のデフォルト）を利用するために
  -- 読み込んでいる。実際の読み込みは mason-lspconfig 設定内の
  -- require("lspconfig") がトリガーになる。
  ...
  --       mason-lspconfig は依然としてサーバー自動インストールに便利
  {
    ...
    config = function(_, opts)
      ...
      vim.diagnostic.config({
        ...
        -- Diagnostic アイコン
        -- NOTE: sign_define() による DiagnosticSign* の定義は 0.12 で廃止された
        signs = {
          text = {
            [severity.ERROR] = " ",
            [severity.WARN] = " ",
        ...
        -- ]d / [d ジャンプ後にフロートで詳細表示
        -- NOTE: jump({ float = true }) は 0.12 で非推奨（→ on_jump）
        jump = {
      ...
      -- -----------------------------------------------------------------------
      -- 各 LSP サーバーの設定
      -- on_attach / capabilities は下のループで一括設定する
      -- -----------------------------------------------------------------------
      local servers = {
        ...
        rust_analyzer = {
          settings = {
            ["rust-analyzer"] = {
              ...
              -- checkOnSave は boolean になった（コマンド指定は check.command）
              checkOnSave = true,
